function [train_err, test_err, train_var, test_var] = learningCurve(X_train_val, Y_train_val, X_test, Y_test, lambda_idx)
% make learning curves for closed form method
...
for i = learning_iters:learning_iters:r
    [train_err(count) test_err(count) train_var(count) test_var(count)] = Ridge_closed_form_test(X_train_val(1:i, :), Y_train_val(1:i, :), X_test, Y_test, lambda_idx);
    count = count + 1;
end
